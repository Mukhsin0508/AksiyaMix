...
def validate_unique_id(value):
    ...
    while Discount.objects.filter(id_generate = value).exists():
...
def validate_video_size(video):
    """ Validate Discount Video size
     The video file size should not exceed 50MB
    """
    # 50MB max size
...
def validate_video_aspect_ratio(video):
    ...
    The video aspect ratio should be 9/16
    """
    clip = VideoFileClip(video.file.path)
    width, height = clip.size
    aspect_ratio = width / height
    if not (aspect_ratio == 9/16):
        raise ValidationError("The video aspect ratio should be 9/16")
